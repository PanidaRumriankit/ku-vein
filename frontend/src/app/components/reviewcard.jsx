...
export default function ReviewCard({ item, page = null, bookmark = false }) {
  ...
  const chips = [
    item.criteria,
    item.classes_type,
    attendant[item.attendances],
    "ยาก: " + efforts[item.efforts],
  ];
  ...
  return (
    <div className="mx-20 my-4 max-w-4xl text-black dark:text-white">
      <fieldset className="border border-gray-300 rounded-md p-4">
        {page === "page" && (
          <legend
            style={{ backgroundColor: color, borderColor: color }}
            className="flex hover:cursor-pointer p-2 border-solid border rounded text-black font-bold dark:text-white"
        ...
        <div className="text-black dark:text-white">
          ...
          {/*grades, date, authors*/}
          <div className="block md:justify-between md:flex text-gray-400">
            <p className="text-left">เกรด: {item.grades}</p>
            <p className="max-w-[50%] text-left md:text-right break-words">
              {formattedDate} โดย:{" "}
              <span
                className={`${!item.is_anonymous ? "cursor-pointer" : ""} break-all`}
